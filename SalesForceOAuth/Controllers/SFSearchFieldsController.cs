...
namespace SalesForceOAuth.Controllers
{
    public class SFSearchFieldsController : ApiController
    {
        ...
        [HttpGet]
        public async System.Threading.Tasks.Task<HttpResponseMessage> GetSearchFieldsById(string Token, string ObjectRef, int FieldId, string callback)
        {
            //check payload if a right jwt token is submitted
            string outputPayload;
            try
            ...
            {
                string urlReferrer = Request.RequestUri.Authority.ToString();
                var FieldsList = Repository.GetSFSearchFieldsById(FieldId, ObjectRef, urlReferrer);
                return MyAppsDb.ConvertJSONPOutput(callback, FieldsList, HttpStatusCode.OK, false);
            }
            catch (Exception ex)
            {
        ...
        [HttpPut]
        public async System.Threading.Tasks.Task<HttpResponseMessage> UpdateSearchFields(FieldsModel ExportFieldData)
        {
            //check payload if a right jwt token is submitted
            string outputPayload;
            try
            ...
            {
                return MyAppsDb.ConvertJSONOutput(ex, "SF Search Update Fields", "Your request isn't authorized!", HttpStatusCode.InternalServerError);
            }
            try
            {
                string urlReferrer = Request.RequestUri.Authority.ToString();
                string message = Repository.UpdateSFSearchFields(ExportFieldData, urlReferrer);
                return MyAppsDb.ConvertJSONOutput(message, HttpStatusCode.OK, false);
            }
            catch (Exception ex)
            {
                return MyAppsDb.ConvertJSONOutput(ex, "SF Search Update Fields", "Unable to update Search Fields", HttpStatusCode.InternalServerError);
            }
        }
